...
const SECOND_FLAG = "ROBO_NAO_AUTORIZADO";
const THIRD_FLAG = "SUPERIORIDADE_VILTRUMITA";
const FOURTH_FLAG = "GUARDIAN{CECIL_SEMPRE_SABE}";
...
const USER_1_PRIVATE_DATA = {
  ...
  acesso: "AUTORIZADO",
  dadosConfidenciais:
    "Relatório de Status Padrão. Nenhuma informação por aqui.",
...
function generateFakeBase64() {
  const fakeText =
    "ContinueProcurando" + Math.random().toString(28).substring(2);
...
app.post("/verificar-flag-02", (req, res) => {
  ...
  if (!flag) {
    return res
      .status(400)
      .json({ success: false, message: "Parâmetro de flag ausente." });
  }
  ...
  if (flag.trim() === SECOND_FLAG) {
    ...
    res.status(200).json({
      success: true,
      message: "Chave Correta! Desafio 02 Concluído.",
      redirectUrl: redirectUrl,
    });
  } else {
    res.status(401).json({
      success: false,
      message: "Chave Incorreta. Tente Novamente!",
    });
  }
});
...
app.post("/verificar-flag-03", (req, res) => {
  ...
  if (flag.trim() === THIRD_FLAG) {
    const redirectUrl = `/d3saf1o-04.html`;
    res.status(200).json({
      success: true,
      message: "Chave Correta!",
      redirectUrl: redirectUrl,
    });
  } else {
    res.status(401).json({
...
//desafio 04
app.get("/d3saf1o-04.html", (req, res) => {
  res.sendFile(path.join(__dirname, "public", "d3saf1o-04.html"));
});
