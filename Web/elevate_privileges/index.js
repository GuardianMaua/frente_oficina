...
const FLAG = process.env.FLAG || "GUARDIAN{EL3V4T3_PR1VS}";
...
app.get("/.well-known/ctf", (_req, res) => {
  const dica =
    'Visite /elevate para ganhar privilégio temporário e depois envie o formulário "Envie seus dados" com role=admin.';
...
app.get("/elevate", (req, res) => {
  ...
  res
    .type("text/plain")
    .send(
      "Privilégio temporário concedido por 10 minutos. Volte à página e envie o formulário com role=admin."
    );
});
...
app.post("/postExample", (req, res) => {
  ...
  if (!name || !email) {
    return res
      .status(400)
      .json({ message: "Dados inválidos: informe name e email." });
  }
