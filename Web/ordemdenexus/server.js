...
app.get("/arquivos/backup_secreto", (req, res) => {
  ...
  fs.readFile(filePath, "utf8", (err, data) => {
    if (err) {
      ...
      return res
        .status(500)
        .send("Erro interno do servidor ao acessar o arquivo.");
    }
...
app.post("/painel_de_acesso", (req, res) => {
  ...
  if (
    username === CREDENCIAIS_CORRETAS.usuario &&
    password === CREDENCIAIS_CORRETAS.senha
  ) {
    // Login bem-sucedido
    ...
    res.send(
      'Login falhou. Credenciais não reconhecidas na Ordem de Nexus. <a href="/painel_de_acesso">Voltar</a>'
    );
  }
});
...
app.get("/home", (req, res) => {
  ...
  if (req.cookies.auth === "authenticated" && userId) {
    ...
    res
      .status(401)
      .send(
        'Acesso negado. Por favor, faça login em <a href="/painel_de_acesso">/painel_de_acesso</a>.'
      );
  }
});
...
app.get("/logout", (req, res) => {
  ...
  res.send(
    'Você foi desconectado do Grupo Secreto. <a href="/">Voltar para a Fachada do Hotel</a>'
  );
});
...
app.listen(port, () => {
  ...
  console.log(
    `Credenciais de Login (Força Bruta): ${CREDENCIAIS_CORRETAS.usuario} / (Senha na wordlist!)`
  );
  console.log(
    `Cookie de Admin (Exploração): ${ADMIN_COOKIE_NAME} = ${ADMIN_COOKIE_VALUE}`
  );
  console.log(`======================================================\n`);
});
